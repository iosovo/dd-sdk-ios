...
public class RUMMonitor: DDRUMMonitor {
    ...
    /// Non-thread-safe global attributes dictionary
    private var unsafeGlobalAttributes = [AttributeKey: AttributeValue]()
    ...
    override public var attributes: [AttributeKey: AttributeValue] {
        return queue.sync {
            self.unsafeGlobalAttributes
        }
    }

    override public func setAttribute(forKey key: AttributeKey, value: AttributeValue?) {
        queue.async {
            self.unsafeGlobalAttributes[key] = value
        }
    }
    ...
    private func aggregate(_ localAttributes: [AttributeKey: AttributeValue]?) -> [AttributeKey: AttributeValue] {
        if let someAttributes = localAttributes {
            return attributes.merging(someAttributes) { $1 }
        }
        return attributes
    }
